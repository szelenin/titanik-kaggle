...
set.seed(415)
model <- train(Survived ~ ., data=tmpTrain, tuneGrid=expand.grid(.mtry=c(1,2,3,4,5)), method="rf", na.action=na.omit, trControl=ctrl)
...
#
predictions <- predict(model, newdata = tmpTest)
